...
This page will be updated to add examples of extracting errors from `TransactionPlanResult` (failed transactions in multi-transaction operations) and to show how to parse program-specific errors from `SolanaError` context when using `client.sendTransaction()`.
