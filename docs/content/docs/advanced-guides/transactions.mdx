...
**Status:** Existing page — significant enrichment planned

This page will be updated to show how `rpcTransactionPlanExecutor` handles blockhash fetching, CU estimation, signing, and confirmation, add references to the plugin-based transaction workflow, add a section about estimating compute units, and document the new unified transaction config setters (`setTransactionMessageComputeUnitLimit`, etc.). The `pipe()` / immutability walkthrough from the current `getting-started/build-transaction.mdx` will be ported to [Kit Without a Client](/docs/advanced-guides/kit-without-a-client).
