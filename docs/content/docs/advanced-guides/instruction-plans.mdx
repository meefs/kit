...
**Status:** Existing page — significant updates planned

This page will be updated to add a section showing `transactionPlanner` and `transactionPlanExecutor` plugins, demonstrate how to swap the executor on an existing RPC client (e.g. replacing the default one from `createClient`), show the connection between instruction plans and `client.sendTransaction()` / `client.sendTransactions()`, and add examples using `rpcTransactionPlanner` and `rpcTransactionPlanExecutor` from `@solana/kit-plugin-rpc`.
