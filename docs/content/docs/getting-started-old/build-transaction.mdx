...
**Content redistributed to:** The `pipe()` / immutability walkthrough and transaction construction patterns will move to [Advanced Guides — Transactions](/docs/advanced-guides/transactions), and the practical token creation steps will move to [Recipes — Creating a Token](/recipes/creating-a-token).
...
// Add the compute unit limit instruction to the transaction message.
const transactionMessageWithComputeUnitLimit = appendTransactionMessageInstruction(
    getSetComputeUnitLimitInstruction({ units: computeUnitsEstimate }),
    transactionMessage,
);
```

Since this is a common operation, let's add a helper function to our `Client` type so we can reuse it any time we need to set a compute unit limit.

First, let's create a new `estimateAndSetComputeUnitLimitFactory` function that wraps the `estimateComputeUnitLimitFactory` to return a function that does everything in the code snippet above.
...
Next, let's use the return type of that function to provide a new `estimateAndSetComputeUnitLimit` function in our `Client` type and export it in our `createClient` helper.
...
// ---cut-before---
export type Client = {
    estimateAndSetComputeUnitLimit: ReturnType<typeof estimateAndSetComputeUnitLimitFactory>; // [!code ++]
    rpc: Rpc<SolanaRpcApi>;
    rpcSubscriptions: RpcSubscriptions<SolanaRpcSubscriptionsApi>;
    wallet: TransactionSigner & MessageSigner;
};
...
export async function createClient(): Promise<Client> {
    if (!client) {
        ...
        // Store the client.
        client = {
            estimateAndSetComputeUnitLimit, // [!code ++]
            rpc,
            rpcSubscriptions,
            wallet,
        };
